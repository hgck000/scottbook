...
- One hundred and five built-in readings: thirty-nine at HSK 1, twenty-four
  each at HSK 2 and HSK 3, and nine each at HSK 4 and HSK 5. Thirty long HSK 1
  stories plus fifteen long stories at each of HSK 2 and HSK 3 contain seventy
  sentences under five connected sections; the earlier forty-five readings
  retain ten sentences under two sections. Every level includes at least three
  readings about clothing, office work, fashion, or design.
...
- HSK 1–5 plus in-progress, completed, and favorite library filters with live
  result counts.
...
The final command writes `artifacts/ScottBook-v0.45.0-android-debug.apk` and
...
```bash
npm run android:device:smoke -- /path/to/ScottBook-v0.45.0-android-debug.apk
```
...
`artifacts/ScottBook-v0.45.0-android-release.apk` plus a checksum/fingerprint
...
Version 0.36 adds nine HSK 5 readings with the same ten-sentence, two-section
structure, including four stories about office work, clothing, or design.
HSK 5 uses a distinct azure accent across filters, cards, progress, and Reader.
The readings remain original adaptations checked against the official HSK 5
exam and vocabulary resources, with complete authored offline assistance. See
[`docs/release/SCOTTBOOK-v0.36.0.md`](docs/release/SCOTTBOOK-v0.36.0.md).

Version 0.37 adds thirty long HSK 1 short stories, each with seventy sentences
across five connected sections. Fifteen stories focus on clothing, office work,
fashion, or design. All word/phrase pinyin and Vietnamese meanings, sentence
translations, and offline Hán-Việt assistance remain available. The generated
library is gzip-compressed inside the local JavaScript bundle before PWA and APK
packaging, so search remains synchronous without multiplying the shipped bundle
by the raw annotation size. See
[`docs/release/SCOTTBOOK-v0.37.0.md`](docs/release/SCOTTBOOK-v0.37.0.md).

Version 0.41 audits the existing 75-reading library without adding content.
It removes irrelevant dictionary senses, fixes neutral-tone and polyphonic
pinyin, gives recurring names and compounds contextual Vietnamese meanings,
and rewrites awkward sentences found in the long HSK 1 stories. Regression
tests now reject dictionary noise and lock the reviewed readings while full
offline Hán-Việt coverage remains required. See
[`docs/release/SCOTTBOOK-v0.41.0.md`](docs/release/SCOTTBOOK-v0.41.0.md).

Version 0.42 keeps that library and every reading feature unchanged while
reducing repeated work. Generated annotations use a shared string table before
gzip, article and vocabulary search reuse normalized indexes, and cross-library
word contexts are indexed only after a word is selected. The initial shell is
also split from the larger offline library chunk so ScottBook can paint a local
...
Version 0.44 finishes the simplified reading interface. It removes redundant
helper, discovery, end-of-article, and Review presentation blocks; keeps the
advanced local-data card hidden without deleting its backup/restore code; and
rebalances the compact Chinese Library hero across desktop and mobile. See
[`docs/release/SCOTTBOOK-v0.44.0.md`](docs/release/SCOTTBOOK-v0.44.0.md).

Version 0.45 adds fifteen original long stories to each of HSK 2 and HSK 3.
Each contains seventy sentences under five connected sections and ships with
offline pinyin, Vietnamese word meanings, sentence translations, and Hán-Việt
assistance. The library now has 105 readings and 4,650 sentences. See
[`docs/release/SCOTTBOOK-v0.45.0.md`](docs/release/SCOTTBOOK-v0.45.0.md).
...
All 105 built-in reference articles and the pinned import dictionary are
...
the v0.45 commit, download the `ScottBook-Android-debug-…` artifact, extract it,
and install `ScottBook-v0.45.0-android-debug.apk`. Android may ask permission to
