...
- Seventy-five built-in readings: thirty-nine at HSK 1 and nine each at HSK
  2 through HSK 5. The thirty new HSK 1 short stories each contain seventy
...
The final command writes `artifacts/ScottBook-v0.44.0-android-debug.apk` and
...
```bash
npm run android:device:smoke -- /path/to/ScottBook-v0.44.0-android-debug.apk
```
...
`artifacts/ScottBook-v0.44.0-android-release.apk` plus a checksum/fingerprint
...
All seventy-five built-in reference articles and the pinned import dictionary are
...
the v0.44 commit, download the `ScottBook-Android-debug-…` artifact, extract it,
and install `ScottBook-v0.44.0-android-debug.apk`. Android may ask permission to
