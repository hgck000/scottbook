...
Download and extract `ScottBook-v0.33.0-android-debug.apk`, open PowerShell in
the ScottBook repository, then run:

```powershell
npm ci
npm run android:device:smoke -- "C:\Users\vligh\Downloads\ScottBook-v0.33.0-android-debug.apk"
```
...
```powershell
npm run android:device:smoke -- "C:\Users\vligh\Downloads\ScottBook-v0.33.0-android-debug.apk" --serial "DEVICE_SERIAL"
```
